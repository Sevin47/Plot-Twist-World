...
const CHANGELOG = [
  ...
  },
  {
    ...
    date: "Jul 28, 2026",
    notes: [
      "Covenants are now offered when you redevelop a tile, not every time a building finishes. One choice per tile, at the moment you've maxed it out and chosen to start over — instead of a decision every few seconds during an upgrade session.",
      "What you sign is permanent. It survives every rebuild after it, forever, for as long as the tile is yours.",
      "It ends when the tile stops being yours: sold on the market, abandoned for the 50% refund, taken from you in a raid, or repossessed for inactivity. Whoever holds it next starts clean and picks their own.",
      "The paid release is gone — giving the tile up is the way out now.",
    ],
  },
  {
    id: "1.25.0",
...
const TIPS = [
  ...
  },
  {
    ...
    text: "Redeveloping brings offers. Every one is a trade — more rent for weaker walls, cheaper builds for lower rent. Take one or none; declining is free. What you sign is permanent for as long as you hold the tile, through every rebuild after this one.",
  },
  {
...
function regionLabel(region) {
  ...
  // PROMINENCE FIRST, proximity only as a fallback. A region is ~150km
  // across and routinely holds one metro plus a dozen small towns, so
  // "whichever city is nearest a sample point" is the wrong question — it
  // named the Cape Peninsula "Paarl" (a town that happens to sit 1.4km
  // from the region's exact midpoint, beating Cape Town's 19km) and
  // Oʻahu "near Wahiawa" instead of Honolulu. Both were the honest answer
  // to the old question and neither is a name any player would use, which
  // made a region genuinely hard to find in the Territory list: you go
  // looking for the city you actually own tiles in and it isn't there.
  //
  // So: score every city in and around the region's real bounds on
  // population, and penalise only the distance it sits OUTSIDE those
  // bounds. Inside cities compete on size alone, which is what makes
  // Cape Town beat Paarl; a metro just over the border still wins against
  // a village inside, which is what stops the naive "biggest one inside"
  // rule from renaming a suburb of somewhere famous after its own village.
  // Ties (the table's populations are bucketed) break toward the middle.
  const lonW = (rx / rn) * 360 - 180, lonE = ((rx + 1) / rn) * 360 - 180;
  const latN = wyToLat(ry / rn), latS = wyToLat((ry + 1) / rn);
  const [cwx, cwy] = centerOfQk(region);
  const clat = wyToLat(cwy), clon = cwx * 360 - 180;
  const cosl = Math.max(0.05, Math.cos((clat * Math.PI) / 180));
  const MARGIN = 0.35; // ~35km of slop around the box, in degrees
  let pick = null, pickScore = -Infinity, pickOut = 0, pickD = Infinity;
  for (let bl = Math.floor((latS - MARGIN) * 2); bl <= Math.floor((latN + MARGIN) * 2); bl++) {
    for (let bg = Math.floor((lonW - MARGIN) * 2); bg <= Math.floor((lonE + MARGIN) * 2); bg++) {
      const bucket = CITY_IDX.get((bl + 200) * 1000 + (bg + 400));
      if (!bucket) continue;
      for (const c of bucket) {
        // km outside the region rectangle — 0 for anything within it
        const oLat = Math.max(latS - c.lat, c.lat - latN, 0) * 110.57;
        const oLon = Math.max(lonW - c.lon, c.lon - lonE, 0) * 111.32 * cosl;
        const outKm = Math.hypot(oLat, oLon);
        if (outKm > MARGIN * 110.57) continue;
        // 15km of "outside" costs one logp unit (~10x population), so a
        // metro can claim a region it spills into but the reach falls off
        // fast: 10x bigger buys ~15km over the border, 100x bigger ~30km.
        // Tuned against Maui — at a shallower penalty Honolulu, 37km away
        // across open ocean on another island, out-scored Kahului sitting
        // inside the region, and named it "near Honolulu".
        const score = c.logp - outKm / 15;
        const d = Math.hypot((c.lon - clon) * cosl, c.lat - clat);
        if (score > pickScore || (score === pickScore && d < pickD)) {
          pick = c; pickScore = score; pickOut = outKm; pickD = d;
        }
      }
    }
  }
  if (pick) {
    const out = pickOut > 0 ? `near ${pick.name}` : pick.name;
    REGION_LABEL_CACHE.set(region, out);
    return out;
  }

  // Nothing in or near the region at all — ocean, desert, ice. Fall back
  // to the nearest city to any of five samples (center + corners, so a
  // distant city still beats a bare coordinate), and finally to
...
for (const [name, la, lo, lp] of CITY_DATA) {
  ...
  // logp is kept on the city, not just folded into R: cityRadiusKm clamps
  // at 42km, so every city above ~6M people ends up with an identical R
  // and it can't be used to tell a metro from a large town. regionLabel
  // below needs that distinction to name a region.
  const c = { name, lat, lon, logp, R: cityRadiusKm(logp) };
...
const COVENANTS = {
  ...
  // LIFETIME: a covenant is permanent for one owner's TENURE. It survives
  // redevelopment and any amount of rebuilding, and ends the moment the
  // tile leaves that player's hands — captured in a raid, sold on the
  // market, abandoned for the 50% refund, or repossessed for inactivity.
  // New owner, clean slate; they draft their own on their first redevelop.
...
function Game({ G, onExit, startFresh, reducedOverride, jumpToQk, onJumpHandled, startTutorial, onTutorialStarted,
  ...
  /* ── territory: which regions this account has unlocked. Pulled at boot
     AND on every resync, not once at boot — unlocked_regions changes from
     outside this session (a region unlocked on a phone while the desktop
     tab sits open, or in a second tab of the same browser), and this used
     to be a boot-only fetch, so those sessions kept rendering the old
     region count until a full reload. An installed PWA can go days without
     one, which made "I unlocked it and it never showed up in World" a
     genuinely unrecoverable-looking bug rather than a refresh away.

     Add-only, deliberately: nothing server-side ever DELETES from
     unlocked_regions (see supabase.sql — unlock_region and the first-tile
     bootstraps are the only writers), so a row missing from a response is
     always a partial/failed read, never a revoked region. Union means a
     failed fetch can't briefly re-fog territory the player owns. ── */
  ...
  useEffect(() => {
    (async () => {
      ...
      // (refreshTerritory now runs on every resync, not just here), never a
  ...
  /* ── covenants: the three-card draft a REDEVELOP earns ──────────────
     ...
     seconds and permanent decisions arrived faster than anyone could care
     about them. Redevelops are rare and earned — you reached the ceiling
     and chose to reset — which is the right cadence for a choice that
     lasts your whole tenure on that tile.

     PULL, NOT PUSH: even at that cadence nothing interrupts. A badge
     counts waiting offers, the tile sheet renders the cards inline, and a
     one-shot TIP teaches it once rather than a modal asking forever.
     ...
     tiles you own, so an unfiltered select IS the personal list) — which
     also picks up anything earned on another device.
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => {
      ...
      n++;
      if (n % 8 === 0) checkAch();
      if (n % 80 === 0) { syncRent(); collectBank(); collectBattles(); refreshCovOffers(); refreshTerritory(); }
  ...
  }, [ready, g, checkAch, syncRent, collectBank, collectBattles, refreshContracts, refreshCollections, refreshCovOffers, refreshTerritory, toast]);
  ...
  const buyUnowned = async (qk) => {
    ...
    const r = data.rarity;
    // freshly claimed land is always covenant-free — a covenant ends with
    // the tenure that signed it, so reclaimed land comes back clean
  ...
  const buyListed = async (qk) => {
    ...
    g.bal -= price;
    // no covenant: buy_listed_tile clears it on transfer, because it was a
    // term of the SELLER's tenure. The buyer keeps the level and prestige
    // they paid for and drafts their own card on their first redevelop.
  ...
  const attackTile = async (qk) => {
    ...
    if (r && r.t[qk]) {
      if (result.won) {
        ...
        delete r.t[qk].p;
        delete r.t[qk].fp;
        // a covenant ends with the tenure it belonged to — the loser's
        // bargain doesn't transfer to whoever took the tile (attack_tile)
        delete r.t[qk].cv;
        delete r.t[qk].cvAt;
        delete r.t[qk].cvm;
      } else {
    ...
    if (result.won) {
      // mirror attack_tile's own bootstrap EXACTLY: it inserts into
      // unlocked_regions only when this capture is the account's
      // first-ever tile. It does NOT grant the region on any later
      // capture, so adding it unconditionally here invented a region the
      // server had never heard of — the World tab would count it until
      // the next reload, then silently drop back. Unreachable through the
      // UI today (the sheet's attack controls are gated on !selLocked, so
      // a cross-border target can't be selected in the first place), but
      // the two sides have to agree regardless of which gate is holding.
      const wasFirstTile = g.own.length === 0;
  ...
  const redevelop = async (qk) => {
    ...
    // The covenant survives redevelopment untouched — it's a term of this
    // owner's tenure, not of the building that just came down.
    ...
    // REDEVELOP IS THE OFFER POINT (see redevelop_tile). The server has
    // just rolled a fresh 3-card offer if this tile had no covenant yet;
    // pick it up so the strip appears in the sheet the player is already
    // looking at.
  ...
  useEffect(() => {
    ...
    const resync = () => {
      ...
      syncRent();
      refreshOwnedTiles();
      // territory, not just tiles: unlocking a region on another device is
      // exactly the "switched devices" case this resync exists for, and it
      // moves the World tab's region list, the Assets grouping, the Market
      // scope and the fine grid's fog all at once
      refreshTerritory();
      collectBank();
      collectBattles();
      refreshContracts();
  ...
  }, [ready, tab, syncRent, refreshOwnedTiles, refreshTerritory, collectBank, collectBattles, ensureRegion]);
  ...
  const assetRegions = tab !== "assets" ? [] : (() => {
    ...
    for (const t of assetsFiltered) {
      ...
      if (!grp) { grp = { region, tiles: [], rent: 0, upgradable: 0, building: 0 }; groups.set(region, grp); }
      grp.tiles.push(t);
      grp.rent += rentOf(t);
      if (canUpgrade(t)) grp.upgradable++;
    }
    // assetsFiltered drops building tiles (they get their own section), so
    // a region whose every tile happens to be under construction produced
    // no group at all and simply disappeared from the list — the region
    // count above it under-reported with it. Count them here so the group
    // survives, and so a region that's half-built reads honestly. Only
    // when no filter is active and there's an actual list to group: with a
    // filter on, a group with zero matching rows would look like the
    // search had failed, and when EVERY tile is building the "everything
    // you own is under construction" line below already says it better
    // than a wall of empty groups would.
    if (!assetsFilterActive && groups.size > 0) {
      for (const t of assetsBuilding) {
        const region = regionOf(t.qk);
        let grp = groups.get(region);
        if (!grp) { grp = { region, tiles: [], rent: 0, upgradable: 0, building: 0 }; groups.set(region, grp); }
        grp.building++;
                ...
                <div>
                  ...
                  {selMine.cv && COVENANTS[selMine.cv] && (
                    <div className="mb-2 rounded-xl p-2.5" style={cardSty}>
                      ...
                      {/* States the exits plainly. A covenant is permanent
                          for as long as this player holds the tile — it
                          survives every rebuild — so the only ways off it
                          are ways of giving the tile up, and hiding that
                          would make the trade dishonest. */}
                      <div className="pt10 mt-1.5" style={{ ...mono, color: C.dim }}>
                        Permanent while you hold this tile — survives redeveloping. It ends only if you sell it, abandon it, or lose it in a raid.
                      </div>
                    </div>
                  )}
                ...
                return (
                  <div key={grp.region} className="mb-2">
                    <button aria-expanded={open}
                      ...
                      {grp.building > 0 && <Chip color={C.amber}>🔨{grp.building}</Chip>}
                    ...
                    {open && (
                      <div className="ml-2.5 mt-2 pl-2.5" style={{ borderLeft: `1px solid ${C.hair}` }}>
                        {grp.tiles.length === 0 ? (
                          <div className="pt11 pb-2" style={{ ...mono, color: C.dim }}>
                            Every tile here is under construction — see 🔨 Building above.
                          </div>
